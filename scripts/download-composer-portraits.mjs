...
async function searchCommons(composer) {
	...
	for (const hit of data?.query?.search || []) {
		...
		if (!title) continue;
		try { return await commonsThumb(title); } catch {}
	}
	throw new Error(`No Commons search hit for ${composer.id}`);
}
async function downloadOne(composer) {
	const dest = join(OUT_DIR, `${composer.id}.jpg`);
	if (await exists(dest)) { console.log(`skip   ${composer.id}`); return true; }
	const attempts = [
		...
		() => fetchBuffer(searchCommons(composer))
	];
	for (const attempt of attempts) {
		try {
			...
			console.log(`ok     ${composer.id} (${buf.byteLength} bytes)`);
			return true;
		} catch (error) { console.warn(`retry  ${composer.id}: ${error?.message || error}`); }
	}
	console.warn(`fail   ${composer.id}`);
	return false;
}
await mkdir(OUT_DIR, { recursive: true });
let ok = 0;
for (const composer of COMPOSERS) if (await downloadOne(composer)) ok += 1;

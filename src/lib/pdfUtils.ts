...
export type PdfPageProxy = {
	...
	render(params: RenderParams): PdfRenderTask;
...
async function readNativePdf(path: string): Promise<Uint8Array> {
...
async function fetchPdfBytes(url: string): Promise<Uint8Array> {
	...
	if (!bytes.byteLength) throw new Error('Fetched PDF was empty');
	return bytes;
}
...
function createPage(document: PdfDocumentRenderer, info: PdfPageInfo): PdfPageProxy {
	return {
		...
		getViewport({ scale }) {
			...
			return {
				scale: safeScale,
				width: info.width * safeScale,
				height: info.height * safeScale
			};
		},
		...
		async getTextContent() {
			const text = document.getPageText ? await document.getPageText(info.index) : '';
...
export async function openPdfBytes(bytes: Uint8Array, id = 'score.pdf'): Promise<OpenedPdf> {
	...
	const proxy: PdfDocumentProxy = {
		numPages: pages.length,
		getPage(pageNumber) {
			const info = pages[pageNumber - 1];
			if (!info) return Promise.reject(new Error(`PDF page ${pageNumber} does not exist`));
			return Promise.resolve(createPage(document, info));
		},
...
export async function openPdf(blob: Blob): Promise<OpenedPdf> {
	...
	return openPdfBytes(new Uint8Array(await blob.arrayBuffer()), blobFilename(blob));
}
...
export async function openPdfFromUrl(url: string): Promise<OpenedPdf> {
	...
	try {
		return await openPdfBytes(await fetchPdfBytes(targetUrl), targetUrl);
	} catch (error) {
		if (isLocalProtocolUrl(targetUrl)) throw error;
		console.warn('URL PDF open failed', error);
		return openPdfBytes(await fetchPdfBytes(targetUrl), targetUrl);
	}
}
...
export async function openPdfSource(source: PdfSource): Promise<OpenedPdf> {
	if (source.nativePath && isTauri()) {
		try {
			return await openPdfBytes(await readNativePdf(source.nativePath), source.nativePath);
		} catch (error) {
			console.warn('Native IPC score read failed; falling back to URL/blob', error);
		}
	}
	if (source.url) {
		try {

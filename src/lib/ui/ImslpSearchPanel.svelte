...
{#if open}
	...
	<div class="panel" role="dialog" aria-modal="true" aria-label="Search IMSLP" tabindex="-1">
		...
		{#if !available}
			<div class="banner warn">IMSLP search and download are available in the Sonora desktop app.</div>
		{:else}
		...
		<div class="body" class:split={!!selected}>
			{#if selected}
				<aside class="context">
					...
					{#if snippetFor(selected)}<p class="snippet">{snippetFor(selected)}</p>{/if}
						...
						<div class="score-grid">
							{#each scores as score (score.filename)}
								<article class="score-card">
									...
									<button type="button" class="add-btn" disabled={downloading === score.filename || !libraryRoot} onclick={() => void doDownload(score)}>
										{#if downloading === score.filename}<Loader2 size={15} class="spin" /> Saving…{:else}<Download size={15} /> Add to library{/if}
									</button>
								</article>
							{/each}
						</div>
			...
			{:else}
				<section class="results">
					...
					{:else}
						<div class="results-head"><h3>Works</h3><span>{hits.length} results</span></div>
						<div class="hit-list">
...
<style>
	.backdrop { position: fixed; inset: 0; z-index: 80; background: rgba(8, 10, 14, 0.62); backdrop-filter: blur(6px); }
	.panel {
		position: fixed; z-index: 90; top: 3vh; left: 50%; transform: translateX(-50%);
		width: min(1180px, calc(100vw - 32px)); height: min(92vh, 900px);
		display: flex; flex-direction: column;
		border: 1px solid var(--sonora-border-strong); border-radius: 18px;
		background: var(--sonora-bg-elevated);
		box-shadow: var(--sonora-shadow-lg), 0 24px 80px rgba(0, 0, 0, 0.35);
		overflow: hidden; color: var(--sonora-text);
	}
	.panel-header {
		flex: 0 0 auto; display: flex; align-items: center; justify-content: space-between; gap: 16px;
		padding: 16px 20px; border-bottom: 1px solid var(--sonora-border);
	...
	.mark {
		...
		background: var(--sonora-accent-soft); color: var(--sonora-accent); flex: 0 0 auto;
	}
	...
	.search-field :global(.search-icon) { position: absolute; left: 12px; color: var(--sonora-text-muted); pointer-events: none; }
	.search-field input {
		width: 100%; height: 42px; padding: 0 14px 0 38px; border: 1px solid var(--sonora-border-strong);
		border-radius: 12px; background: var(--sonora-bg-workspace); color: var(--sonora-text); font-size: 14px;
	}
	...
	.banner { margin: 12px 20px 0; padding: 10px 12px; border-radius: 10px; font-size: 13px; line-height: 1.4; }
	.banner.error { background: color-mix(in srgb, #ff5c5c 14%, transparent); color: #ffb4b4; border: 1px solid color-mix(in srgb, #ff5c5c 35%, transparent); }
	.banner.ok { background: color-mix(in srgb, #3ecf8e 12%, transparent); color: #b6f0d2; border: 1px solid color-mix(in srgb, #3ecf8e 30%, transparent); }
	.banner.warn { background: color-mix(in srgb, #f0c14b 12%, transparent); color: #f5e0a0; border: 1px solid color-mix(in srgb, #f0c14b 30%, transparent); }
	...
	.back { border: 0; background: transparent; color: var(--sonora-accent); font-size: 13px; font-weight: 600; padding: 0; margin-bottom: 12px; cursor: pointer; }
	...
	.external { display: inline-flex; align-items: center; gap: 6px; margin-top: 14px; font-size: 13px; color: var(--sonora-accent); text-decoration: none; }
	...
	.results-head, .scores-head { display: flex; align-items: baseline; justify-content: space-between; gap: 12px; margin-bottom: 12px; }
	.results-head h3, .scores-head h3 { margin: 0; font-size: 13px; font-weight: 650; letter-spacing: 0.04em; text-transform: uppercase; color: var(--sonora-text-faint); }
	...
	.hit {
		...
		border: 1px solid transparent; border-radius: 12px; background: transparent; color: inherit; text-align: left; cursor: pointer;
	}
	.hit:hover { background: var(--sonora-bg-hover); border-color: var(--sonora-border); }
	.hit-icon { width: 36px; height: 36px; display: grid; place-items: center; border-radius: 10px; background: var(--sonora-bg-active); color: var(--sonora-text-muted); flex: 0 0 auto; }
	...
	.hit-body .snippet { margin: 4px 0 0; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
	.score-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 14px; }
	.score-card {
		display: flex; flex-direction: column; gap: 10px; padding: 12px; border: 1px solid var(--sonora-border);
		border-radius: 14px; background: color-mix(in srgb, var(--sonora-bg-workspace) 65%, transparent);
	}
	.thumb { aspect-ratio: 3 / 4; border-radius: 10px; overflow: hidden; background: var(--sonora-bg-active); border: 1px solid var(--sonora-border); }
	.thumb img { width: 100%; height: 100%; object-fit: cover; object-position: top center; display: block; }
	.thumb-fallback { width: 100%; height: 100%; display: grid; place-items: center; color: var(--sonora-text-faint); }
	.score-meta { display: flex; flex-direction: column; gap: 4px; min-height: 56px; }
	.score-meta strong { font-size: 13px; font-weight: 650; line-height: 1.3; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
	.desc, .editor { font-size: 12px; color: var(--sonora-text-muted); line-height: 1.35; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
	.editor { color: var(--sonora-text-faint); }
	.add-btn {
		height: 34px; display: inline-flex; align-items: center; justify-content: center; gap: 7px;
		border: 1px solid var(--sonora-border-strong); border-radius: 10px; background: var(--sonora-bg-elevated);
		color: var(--sonora-text); font-size: 12.5px; font-weight: 600; cursor: pointer;
	}
	.add-btn:hover:not(:disabled) { background: var(--sonora-bg-hover); border-color: color-mix(in srgb, var(--sonora-accent) 40%, var(--sonora-border-strong)); }
	.add-btn:disabled { opacity: 0.55; cursor: not-allowed; }
	.empty-state { display: grid; place-items: center; gap: 10px; min-height: 220px; color: var(--sonora-text-muted); text-align: center; padding: 24px; }

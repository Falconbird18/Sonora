<script lang="ts">
	...
	const pages = $derived(dual && pdf ? [Math.max(1, page % 2 === 0 ? page : page - 1), Math.min(pdf.numPages, (page % 2 === 0 ? page : page - 1) + 1)] : [page]);
	...
	onMount(() => {
		...
		const key = (event: KeyboardEvent) => {
			...
			else if (event.key === 'Escape') { settingsOpen = false; searchOpen = false; annotationOpen = false; tool = 'move'; }
		};
	...
	async function render() {
		...
		try {
			...
			for (let index = 0; index < pages.length; index++) {
				await renderPage(pages[index], index, currentGeneration);
				if (currentGeneration !== generation) return;
			}
		} catch (reason) {
			if (!(reason instanceof Error && reason.name === 'RenderingCancelledException') && currentGeneration === generation) {
	...
	async function renderPage(number: number, index: number, currentGeneration: number) {
		if (!pdf || !host) return;
		...
		scale = Math.max(0.2, Math.min(3, Math.min(scale, pixelScale)));
		...
		await task.promise;
		if (currentGeneration === generation) redraw(number, inkCanvas);
	...
	function canvasPoint(point: Point, canvas: HTMLCanvasElement) {
		const rect = canvas.getBoundingClientRect();
		return { x: point.x * rect.width, y: point.y * rect.height };
	}
	function choose(nextTool: Tool) {
		tool = nextTool;
		annotationOpen = nextTool !== 'move';
	}
	function begin(event: PointerEvent, number: number, canvas: HTMLCanvasElement) {
		if (!annotationOpen || reading) return;
		event.preventDefault();
		canvas.setPointerCapture(event.pointerId);
		const point = position(event, canvas);
		if (tool === 'symbol') {
			const stamp: SymbolStamp = { id: crypto.randomUUID(), symbol: selectedSymbol.glyph, label: selectedSymbol.name, x: point.x, y: point.y, fontSize: symbolSize, color };
	...
	function move(event: PointerEvent) {
		...
		for (const pointEvent of event.getCoalescedEvents?.() || [event]) {
			if (drawing.stroke) {
				...
				if (stroke) {
					if (stroke.kind === 'line' || stroke.kind === 'arrow') stroke.points = [stroke.points[0], position(pointEvent, drawing.canvas)];
					else stroke.points.push(position(pointEvent, drawing.canvas));
				}
	...
	function redraw(number: number, canvas: HTMLCanvasElement | null) {
		...
		for (const stamp of stamps[number] || []) {
			const p = canvasPoint({ x: stamp.x, y: stamp.y }, canvas);
			context.save();
		...
		for (const note of notes[number] || []) {
			const p = canvasPoint({ x: note.x, y: note.y }, canvas);
			context.save();
	...
	function drawStroke(context: CanvasRenderingContext2D, stroke: Stroke, canvas: HTMLCanvasElement) {
		...
		const points = stroke.points.map((point) => canvasPoint(point, canvas));
		context.save();
	...
	function checkpoint(number: number) {
		...
		const old = histories[number] || [current];
		const index = historyIndex[number] ?? old.length - 1;
		const nextHistory = [...old.slice(0, index + 1), current].slice(-80);
		histories[number] = nextHistory;
	...
	function applySnapshot(number: number, state: Snapshot) {
		...
		const canvas = number === pages[0] ? leftInk : rightInk;
		redraw(number, canvas);
		scheduleSave(number);
	}
	function undo() { const index = historyIndex[page] ?? 0; if (index <= 0) return; historyIndex[page] = index - 1; applySnapshot(page, histories[page][index - 1]); }
	function redo() { const index = historyIndex[page] ?? 0; const list = histories[page] || []; if (index >= list.length - 1) return; historyIndex[page] = index + 1; applySnapshot(page, list[index + 1]); }
	function scheduleSave(number: number) {
		const oldTimer = saveTimers.get(number); if (oldTimer) clearTimeout(oldTimer);
		saveTimers.set(number, setTimeout(() => void saveAnnotations(number), 250));
	}
	async function saveAnnotations(number: number) {
		await db.annotations.put({ id: `${score.id}:${number}`, scoreId: score.id, pageNum: number, strokes: $state.snapshot(strokes[number] || []), stamps: $state.snapshot(stamps[number] || []), notes: $state.snapshot(notes[number] || []) });
	}
	...
	function setFit(value: Fit) { fit = value; if (value !== 'free') zoom = 1; persistPrefs(); void render(); }
	function persistPrefs() { localStorage.setItem(prefs, JSON.stringify({ bookmarked, dual, zoom, fit, visible })); }
...
<div class="viewer" class:reading class:fullscreen={fullScreen} bind:this={host}>
	<header class="topbar" class:hidden={reading || !controls}>
		...
		<div class="topbar-right">
			<button class:active={bookmarked} class="icon-button" title="Bookmark score" aria-label="Bookmark score" onclick={toggleBookmark}>★</button>
			...
			<button class="icon-button" title="Toggle reading mode" aria-label="Toggle reading mode" onclick={() => (reading = !reading)}><EyeOff size={18} /></button>
		</div>
	</header>
	...
	{#if searchOpen && !reading}
		<div class="search-panel">
			<Search size={17} />
			<input autofocus bind:value={searchText} placeholder="Find text in this score…" onkeydown={(e) => e.key === 'Enter' && searchPdf()} />
			<button class="text-button" onclick={searchPdf}>Find</button>
			<span>{searchStatus}</span>
			<button class="icon-button" onclick={() => (searchOpen = false)} aria-label="Close search"><X size={17} /></button>
		</div>
	{/if}
		...
		<div class="pages" class:dual>
			{#each pages as number, index}
				<div class="page-shell">
					<canvas class="pdf-canvas" bind:this={index === 0 ? leftPdf : rightPdf}></canvas>
					<canvas class="ink-canvas" class:interactive={annotationOpen && !reading} bind:this={index === 0 ? leftInk : rightInk}
						onpointerdown={(event) => index === 0 ? begin(event, number, leftInk!) : begin(event, number, rightInk!)}
						onpointermove={move} onpointerup={end} onpointercancel={end} onpointerleave={(event) => { if (drawing && drawing.pointerId === event.pointerId && tool === 'eraser') end(); }}></canvas>
				</div>
			{/each}
		</div>
		{#if loading}<div class="loading"><span></span>Loading score…</div>{/if}
		{#if error}<div class="error"><strong>Unable to display this page</strong><span>{error}</span></div>{/if}
	</main>

	{#if !reading && controls}
		<aside class="annotation-bar" class:open={annotationOpen}>
			<div class="tool-group">
				<button class:active={tool === 'move'} class="tool-button" title="Pointer / pan (V)" onclick={() => choose('move')}><MousePointer2 size={18} /><span>Move</span></button>
				...
				<button class:active={tool === 'arrow'} class="tool-button" title="Arrow" onclick={() => choose('arrow')}><ChevronRight size={18} /><span>Arrow</span></button>
				<button class:active={tool === 'eraser'} class="tool-button" title="Erase annotations (E)" onclick={() => choose('eraser')}><Eraser size={18} /><span>Erase</span></button>
			...
			<div class="tool-group compact">
				<button class="icon-button" title="Undo" aria-label="Undo" disabled={!canUndo} onclick={undo}><Undo2 size={18} /></button>
				<button class="icon-button" title="Redo" aria-label="Redo" disabled={!canRedo} onclick={redo}><Redo2 size={18} /></button>
				<div class="color-row">{#each colors as swatch}<button class="swatch" class:selected={color === swatch} style={`--swatch:${swatch}`} title={`Color ${swatch}`} onclick={() => (color = swatch)}></button>{/each}</div>
				<label class="range-label">Size <input type="range" min="1" max="14" step="1" bind:value={width} /></label>
			</div>
		</aside>
	{/if}

	{#if annotationOpen && tool === 'symbol' && !reading}
		<section class="symbol-palette">
			<div class="palette-header"><div><strong>Musical symbols</strong><span>Rendered with Leland</span></div><input bind:value={symbolSearch} placeholder="Search symbols" /></div>
			...
			<div class="palette-footer"><span>Selected: {selectedSymbol.name}</span><label>Size <input type="range" min="18" max="72" bind:value={symbolSize} /></label></div>
		</section>
	{/if}

	<footer class="bottombar" class:hidden={reading || !controls}>
		<div class="footer-section"><button class="icon-button" title="Fit page" aria-label="Fit page" class:active={fit === 'page'} onclick={() => setFit('page')}>▣</button><button class="icon-button" title="Fit width" aria-label="Fit width" class:active={fit === 'width'} onclick={() => setFit('width')}>↔</button><button class="icon-button" title="Zoom out" aria-label="Zoom out" onclick={() => setZoom(zoom - 0.1)}><ZoomOut size={17} /></button><span class="zoom-value">{Math.round(zoom * 100)}%</span><button class="icon-button" title="Zoom in" aria-label="Zoom in" onclick={() => setZoom(zoom + 0.1)}><ZoomIn size={17} /></button></div>
		<div class="footer-section"><button class:active={dual} class="text-button" onclick={() => { dual = !dual; persistPrefs(); void render(); }}>{dual ? 'Single page' : 'Two pages'}</button><button class="icon-button" title="Toggle annotations" aria-label="Toggle annotations" onclick={() => { visible = !visible; persistPrefs(); void render(); }}>{visible ? <Eye size={17} /> : <EyeOff size={17} />}</button><button class="icon-button" title="Fullscreen" aria-label="Fullscreen" onclick={toggleFullScreen}>{fullScreen ? <Minimize2 size={17} /> : <Maximize2 size={17} />}</button><button class="icon-button" title="Viewer settings" aria-label="Viewer settings" onclick={() => (settingsOpen = !settingsOpen)}>⋯</button></div>
	</footer>

	{#if settingsOpen}
		<div class="settings-card">
			<strong>Viewer</strong>
			<label><input type="checkbox" bind:checked={controls} /> Show controls</label>
			<label><input type="checkbox" bind:checked={dual} /> Two-page view</label>
			<label><input type="checkbox" bind:checked={visible} /> Show annotations</label>
			<label>Text size <input type="range" min="10" max="36" bind:value={textSize} /></label>
			<button class="text-button" onclick={() => { settingsOpen = false; persistPrefs(); void render(); }}>Done</button>
		</div>
	{/if}
</div>
...
<style>
	...
	.viewer { --bg:#121210; --panel:#1a1a17; --panel2:#20201c; --line:rgba(255,255,255,.09); position:relative; width:100%; height:100%; min-height:0; overflow:hidden; background:var(--bg); color:#f5f5f4; font-family:Inter,ui-sans-serif,system-ui,sans-serif; }
	.viewer.fullscreen { position:fixed; inset:0; z-index:100; }
	.topbar,.bottombar { position:relative; z-index:20; display:flex; align-items:center; justify-content:space-between; gap:12px; padding:10px 14px; background:rgba(26,26,23,.94); border-bottom:1px solid var(--line); backdrop-filter:blur(18px); }
	.bottombar { border-top:1px solid var(--line); border-bottom:0; min-height:56px; }
	.topbar-left,.topbar-right,.page-controls,.footer-section,.tool-group,.color-row { display:flex; align-items:center; gap:6px; }
	.topbar-left { min-width:0; flex:1; }.topbar-right { flex:1; justify-content:flex-end; }.score-title { display:flex; flex-direction:column; min-width:0; margin-left:4px; }.score-title strong { white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:min(42vw,520px); font-size:14px; }.score-title span { color:#8e8e86; font-size:11px; margin-top:2px; }
	.icon-button,.tool-button,.text-button { border:1px solid transparent; color:#b7b7ae; background:transparent; border-radius:10px; cursor:pointer; transition:background .15s ease,color .15s ease,border-color .15s ease,transform .12s ease; }.icon-button { width:36px; height:36px; display:grid; place-items:center; }.icon-button:hover,.tool-button:hover,.text-button:hover { color:#fff; background:rgba(255,255,255,.07); }.icon-button:active,.tool-button:active { transform:scale(.96); }.icon-button.active,.tool-button.active,.text-button.active { color:#fff; background:#35352f; border-color:rgba(255,255,255,.1); }.icon-button:disabled { opacity:.3; cursor:not-allowed; }.text-button { padding:8px 11px; font-size:12px; }
	.page-controls { justify-content:center; }.page-controls input { width:48px; height:34px; border:1px solid var(--line); background:#11110f; color:#fff; border-radius:9px; text-align:center; outline:none; }.page-controls input:focus { border-color:#a8a8a0; }.page-controls span { color:#77776f; font-size:12px; }
	...
	.loading,.error { position:absolute; left:50%; top:50%; transform:translate(-50%,-50%); display:flex; flex-direction:column; align-items:center; gap:7px; padding:14px 18px; border:1px solid var(--line); border-radius:14px; background:rgba(26,26,23,.92); box-shadow:0 15px 40px rgba(0,0,0,.35); font-size:12px; color:#b8b8b0; }.loading span { width:18px; height:18px; border:2px solid #66665e; border-top-color:#fff; border-radius:50%; animation:spin .8s linear infinite; }.error strong { color:#fff; }.error span { max-width:340px; text-align:center; line-height:1.45; }@keyframes spin { to{transform:rotate(360deg)} }
	.annotation-bar { position:absolute; z-index:30; left:50%; bottom:68px; transform:translateX(-50%); display:flex; align-items:center; gap:7px; max-width:calc(100% - 24px); padding:7px; border:1px solid var(--line); border-radius:16px; background:rgba(28,28,25,.96); box-shadow:0 14px 45px rgba(0,0,0,.35); backdrop-filter:blur(18px); }.tool-button { min-width:48px; padding:8px 9px; display:flex; flex-direction:column; align-items:center; gap:3px; font-size:9px; }.divider { width:1px; height:32px; background:var(--line); }.tool-group.compact { gap:4px; }.color-row { margin-left:4px; }.swatch { width:17px; height:17px; border-radius:50%; border:2px solid transparent; background:var(--swatch); cursor:pointer; box-shadow:0 0 0 1px rgba(255,255,255,.1); }.swatch.selected { border-color:#fff; box-shadow:0 0 0 1px #111; }.range-label { display:flex; align-items:center; gap:5px; color:#8f8f87; font-size:10px; white-space:nowrap; }.range-label input { width:70px; }
	...
	.settings-card { position:absolute; z-index:50; right:12px; bottom:68px; width:230px; display:flex; flex-direction:column; gap:11px; padding:15px; background:#22221e; border:1px solid var(--line); border-radius:14px; box-shadow:0 18px 45px rgba(0,0,0,.4); font-size:11px; }.settings-card label { display:flex; align-items:center; justify-content:space-between; gap:8px; color:#aaa9a0; }.settings-card input[type=range] { width:90px; }.hidden { display:none !important; }.reading .workspace { inset:0; }.reading .pages { padding:20px; }.reading .ink-canvas { pointer-events:none !important; }
	...
	@media (max-width:620px) { .score-title { display:none; }.topbar { padding:8px; }.workspace { padding:14px; }.symbol-palette { max-height:62vh; }.palette-header input { width:120px; }.category-row { overflow-x:auto; }.bottombar { padding:8px; }.footer-section .text-button { display:none; } }
	@media print { .topbar,.bottombar,.annotation-bar,.symbol-palette,.search-panel,.settings-card,.loading,.error { display:none !important; }.workspace { position:static; overflow:visible; padding:0; background:#fff; }.viewer { height:auto; overflow:visible; background:#fff; }.pages { display:block; }.page-shell { box-shadow:none; page-break-after:always; }.ink-canvas { pointer-events:none; } }
</style>

<script lang="ts">
	...
	import type { PdfDocumentProxy, PdfRenderTask } from './pdfUtils';
	import {
		ArrowLeft,
		ArrowUpRight,
		Bookmark,
	...
	async function paintPage(pdfPage: ReturnType<NonNullable<PdfDocumentProxy['getPage']> extends Promise<infer P> ? () => P : never>, number: number, index: number, scale: number, dpr: number, current: number) {
	...
	function schedulePrefetch() {
		...
		prefetchTimer = setTimeout(() => {
			...
			for (const number of upcoming) if (number >= 1 && number <= pdf.numPages) void pdf.getPage(number).catch(() => {});
		}, 60);
	}

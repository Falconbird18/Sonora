...
export async function syncAllFolders(force = false) {
	...
	// Always resync when any score is missing nativePath (zero-copy migration).
	let needsPathBackfill = force;
	if (!needsPathBackfill && folder.nativePath) {
		try {
			const sample = await db.scores
	...
	if (
		...
		!needsPathBackfill &&
		folder.lastSyncedAt &&
		Date.now() - folder.lastSyncedAt < 60_000
	) {
...
/**
 * Resolve a loadable PDF source for a score.
 * Desktop prefers asset-protocol URLs (zero-copy). Browser uses stored blobs.
 * Reconstructs absolute paths from folder + sourcePath when nativePath is missing
 * (e.g. library rows created before the zero-copy migration).
 */
export function resolveScoreSource(score: ScoreItem, folder?: FolderSource): {
	...
	// Explicit URL wins (already prepared).
	if (score.pdfUrl && score.pdfUrl.length > 0) {
		return { url: score.pdfUrl, blob: score.pdfBlob, nativePath: score.nativePath };
	}

	// Absolute native path → asset protocol URL.
	if (score.nativePath && isTauri()) {
		return {
			url: nativeFileUrl(score.nativePath),
			nativePath: score.nativePath,
			blob: score.pdfBlob
		};
	}

	// Reconstruct from folder root + relative path (covers older DB rows).
	if (isTauri() && score.sourcePath) {
		const root = folder?.nativePath;
		if (root) {
			const absolute = joinNativePath(root, score.sourcePath);
			return {
				url: nativeFileUrl(absolute),
				nativePath: absolute,
				blob: score.pdfBlob
			};
		}
	...
	// Browser / fallback: in-memory blob.
	if (score.pdfBlob && score.pdfBlob.size > 0) {
		return { blob: score.pdfBlob, nativePath: score.nativePath };
	}

	return {};
}

/** Async resolve that always tries to load the library folder from IndexedDB. */
export async function resolveScoreSourceAsync(score: ScoreItem): Promise<{
	url?: string;
	blob?: Blob;
	nativePath?: string;
}> {

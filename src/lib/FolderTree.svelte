	...
	{#each nodes as node (node.id)}
		...
		<div class="folder-branch">
			<div
				...
				ondragleave={() => {
					if (dragOverId === node.id) dragOverId = '';
				}}
				ondrop={(event) => drop(node, event)}>
				<button
					class="disclosure"
				...
				<button
					...
					aria-label={`${node.name}${node.scoreCount ? `, ${node.scoreCount} scores` : ''}`}
					onclick={() => onSelect(node)}
					onkeydown={(event) => handleKeydown(event, node)}>
					{#if isExpanded && hasChildren}<FolderOpen size={17} />{:else}<Folder
							size={17} />{/if}
					<span>{node.name}</span>
				...
				<button
					...
					onclick={() => onMenu(node)}><MoreHorizontal size={17} /></button>
			</div>

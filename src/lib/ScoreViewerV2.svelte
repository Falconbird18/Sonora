<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			console.error(e);
			error =
				'This score could not be opened. The PDF may be damaged or unsupported.';
		} finally {
			loading = false;
		}
		return () => {
	...
	async function renderSinglePage(
		...
		const width = isDualPage
			? Math.max(320, (mainContainerRef.clientWidth - 120) / 2)
			: Math.max(320, mainContainerRef.clientWidth - 72);
		...
		const viewport = page.getViewport({
			scale: Math.max(0.1, scale * zoomLevel)
		});
		...
		await page.render({ canvasContext: pdfCanvas.getContext('2d')!, viewport })
			.promise;
	...
	function redrawForPage(page: number) {
		...
		if (isDualPage && page === currentPage + 1)
			redrawOverlay(page, rightDrawCanvas);
	}
	...
	function redrawOverlay(page: number, canvas: HTMLCanvasElement | null) {
		...
		for (const stroke of annotations[page] || []) {
			if (stroke.points.length === 1) {
				...
				ctx.beginPath();
				ctx.arc(p.x, p.y, Math.max(1.5, stroke.width / 2), 0, Math.PI * 2);
				ctx.fillStyle =
					stroke.tool === 'highlighter' ? 'rgba(250,204,21,.4)' : stroke.color;
				ctx.fill();
				continue;
			}
			ctx.save();
	...
	function eraseAt(
		...
		stamps[page] = (stamps[page] || []).filter(
			(s) => Math.hypot(s.x - x, s.y - y) > radius + 12
		);
		notes[page] = (notes[page] || []).filter(
			(n) => Math.hypot(n.x - x, n.y - y) > radius + 12
		);
		redrawOverlay(page, canvas);
		persist(page);
	}
...
<div
	...
	<header
		class="h-14 shrink-0 flex items-center justify-between gap-3 px-3 sm:px-5 bg-neutral-900/95 backdrop-blur-xl border-b border-neutral-800 z-20">
		<div class="flex items-center gap-2 min-w-0">
			<button
				onclick={onBack}
				class="p-2 rounded-xl text-neutral-400 hover:text-white hover:bg-neutral-800"
				title="Back to library"><ArrowLeft size={18} /></button>
			<div class="min-w-0 hidden sm:block">
				<div class="text-sm font-semibold truncate max-w-64">{score.title}</div>
		...
		<div class="flex items-center gap-1.5">
			<button
				...
				class="p-2 rounded-xl hover:bg-neutral-800 text-neutral-400 hover:text-white disabled:opacity-30"
				><ChevronLeft size={18} /></button>
			<div
				class="flex items-center gap-1 text-xs text-neutral-400 min-w-20 justify-center">
				<input
					aria-label="Page"
					type="number"
					min="1"
					...
					class="w-10 bg-neutral-950 border border-neutral-800 rounded-lg px-1.5 py-1 text-center text-neutral-200" /><span
					>/ {totalPages}</span>
			</div>
			<button
				onclick={nextPage}
		...
		<div class="flex items-center gap-1.5">
			<div
				class="hidden md:flex items-center bg-neutral-950 border border-neutral-800 rounded-xl p-0.5">
				<button
					onclick={() => {
				...
				<span class="text-[11px] font-mono w-10 text-center text-neutral-300"
				...
				<button
					...
					class="p-1.5 text-neutral-500 hover:text-white border-l border-neutral-800 ml-1"
					><RotateCcw size={14} /></button>
			</div>
			<button
				onclick={() => (isAnnotationToolOpen = !isAnnotationToolOpen)}
				class="p-2 rounded-xl border {isAnnotationToolOpen
					? 'bg-blue-600/20 border-blue-500/50 text-blue-300'
					: 'bg-neutral-950 border-neutral-800 text-neutral-400 hover:text-white'}"
				title="Annotations"><Pencil size={17} /></button>
			<button
				onclick={() => {
					isDualPage = !isDualPage;
					renderPages();
				}}
				class="p-2 rounded-xl border border-neutral-800 bg-neutral-950 text-neutral-400 hover:text-white"
				title="Toggle facing pages"
				>{#if isDualPage}<Columns size={17} />{:else}<Square
						size={17} />{/if}</button>
		</div>
	</header>
	...
	{#if isAnnotationToolOpen}
		<div
			class="absolute top-16 left-1/2 -translate-x-1/2 z-30 max-w-[calc(100%-1rem)] bg-neutral-900/95 backdrop-blur-xl border border-neutral-700 rounded-2xl shadow-2xl p-2 flex flex-wrap items-center justify-center gap-1.5">
			<button
				onclick={() => (activeTool = 'pen')}
				class="tool {activeTool === 'pen' ? 'active' : ''}"
				title="Pen (P)"><Pencil size={16} /></button>
			<button
				onclick={() => (activeTool = 'highlighter')}
				class="tool {activeTool === 'highlighter' ? 'active' : ''}"
				title="Highlighter (H)"><Highlighter size={16} /></button>
			<button
				onclick={() => (activeTool = 'eraser')}
				class="tool {activeTool === 'eraser' ? 'active' : ''}"
				title="Eraser (E)"><Eraser size={16} /></button>
			<button
				onclick={() => (activeTool = 'stamp')}
				class="tool {activeTool === 'stamp' ? 'active' : ''}"
				title="Musical symbol"><Music2 size={16} /></button>
			<button
				onclick={() => (activeTool = 'text')}
				class="tool {activeTool === 'text' ? 'active' : ''}"
				title="Text note"><Type size={16} /></button>
			<span class="w-px h-6 bg-neutral-700 mx-1"></span>
			{#each QUICK_COLORS as color}<button
					onclick={() => (penColor = color)}
					class="w-5 h-5 rounded-full border-2 {penColor === color
						? 'border-white scale-110'
						: 'border-neutral-600'}"
					style={`background:${color}`}></button
				>{/each}
			<label class="flex items-center gap-1.5 text-xs text-neutral-400 px-1"
				><span>Size</span><input
					type="range"
					min="1"
					max="12"
					bind:value={penWidth} /></label>
			<button
				onclick={() => undo(currentPage)}
				disabled={!historyStack[currentPage]?.length}
				class="tool"
				title="Undo"><Undo size={16} /></button>
			<button
				onclick={() => redo(currentPage)}
				disabled={!redoStack[currentPage]?.length}
				class="tool"
				title="Redo"><Redo size={16} /></button>
			{#if activeTool === 'stamp'}
				<select
					bind:value={selectedSymbol}
					class="bg-neutral-950 border border-neutral-700 rounded-lg text-sm px-2 py-1.5"
					><option value="𝄐">𝄐 Fermata</option
					>{#each MUSICAL_SYMBOLS as item}<option value={item[0]}
	...
	<main
		...
		class="relative flex-1 overflow-auto flex items-center justify-center p-3 sm:p-5 bg-neutral-950">
		{#if loading}
			<div
		...
		{:else if error}
			<div class="max-w-md text-center">
				<div class="text-red-400 mb-2">Unable to open score</div>
				<p class="text-sm text-neutral-500 mb-4">{error}</p>
				<button
					onclick={onBack}
					class="px-4 py-2 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-sm"
					>Return to library</button>
			</div>
		{:else}
			<button
				onclick={previousPage}
				disabled={currentPage <= 1 || isAnnotationToolOpen}
				class="page-zone left-0"><ChevronLeft size={32} /></button>
			<button
				onclick={nextPage}
				disabled={currentPage >= totalPages || isAnnotationToolOpen}
				class="page-zone right-0"><ChevronRight size={32} /></button>
			<div class="flex gap-4 items-center justify-center max-w-full">
				<div
					class="relative shadow-2xl bg-white {filterMode === 'sepia'
					...
					<canvas
						...
						onpointerdown={(e) =>
							leftDrawCanvas && pointerDown(e, currentPage, leftDrawCanvas)}
						onpointermove={(e) =>
							leftDrawCanvas && pointerMove(e, currentPage, leftDrawCanvas)}
						onpointerup={(e) =>
							leftDrawCanvas && finishDrawing(e, currentPage, leftDrawCanvas)}
						onpointercancel={(e) =>
							leftDrawCanvas && cancelDrawing(e, currentPage, leftDrawCanvas)}
						class="absolute inset-0 touch-none {isAnnotationToolOpen
							? 'cursor-crosshair'
				...
				{#if isDualPage && currentPage + 1 <= totalPages}
					<div
						...
						<canvas
							...
							onpointerdown={(e) =>
								rightDrawCanvas &&
								pointerDown(e, currentPage + 1, rightDrawCanvas)}
							onpointermove={(e) =>
								rightDrawCanvas &&
								pointerMove(e, currentPage + 1, rightDrawCanvas)}
							onpointerup={(e) =>
								rightDrawCanvas &&
								finishDrawing(e, currentPage + 1, rightDrawCanvas)}
							onpointercancel={(e) =>
								rightDrawCanvas &&
								cancelDrawing(e, currentPage + 1, rightDrawCanvas)}
							class="absolute inset-0 touch-none {isAnnotationToolOpen
								? 'cursor-crosshair'
								: 'pointer-events-none'}"></canvas>
					</div>
	...
	<footer
		class="h-11 shrink-0 flex items-center justify-between px-4 bg-neutral-900/90 border-t border-neutral-800 text-[11px] text-neutral-500">
		...
		<div class="flex items-center gap-1">
			<button
				...
				class="px-2 py-1 rounded-lg {fitMode === 'height'
			...
			<button
				...
				class="px-2 py-1 rounded-lg {fitMode === 'width'
					? 'bg-neutral-800 text-neutral-200'
					: ''}">Fit width</button>
			<button
				onclick={() => {
				...
				class="px-2 py-1 rounded-lg {fitMode === 'page'
					? 'bg-neutral-800 text-neutral-200'
					: ''}">Fit page</button>
		</div>
		<div class="flex items-center gap-1">
...
<style>
	...
	.page-zone:hover:not(:disabled) {
		opacity: 1;
		background: linear-gradient(90deg, rgba(23, 23, 23, 0.5), transparent);
	}
	.page-zone.right {
		right: 0;
		justify-content: flex-end;
	}
	.page-zone.right:hover:not(:disabled) {
		background: linear-gradient(270deg, rgba(23, 23, 23, 0.5), transparent);
	}
	.page-zone:disabled {
		pointer-events: none;
	}
